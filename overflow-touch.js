...
function setIosOverlay(element_id)
{
	...
	if (is_ios)
	{
		...
		protip.style.top = overflow_element.offsetTop + 'px'; //window.getComputedStyle(overflow_element,null).getPropertyValue('top');
		protip.style.left = overflow_element.offsetLeft + 'px'; //window.getComputedStyle(overflow_element,null).getPropertyValue('left');
